...
from jirassicpack.cli import ensure_output_dir, print_section_header, celebrate_success, retry_or_skip, logger, redact_sensitive, get_valid_user
from jirassicpack.utils import validate_required, validate_date, error, info, spinner, info_spared_no_expense, prompt_with_validation, build_context, render_markdown_report, contextual_log, get_option
...
def write_team_analytics_file(filename: str, start_date: str, end_date: str, workload: Dict[str, int], user_email=None, batch_index=None, unique_suffix=None, context=None) -> None:
    """
    Write the team analytics report to a Markdown file.
    Includes a table of issue counts per user and highlights the most/least loaded team members.
    """
    try:
        with open(filename, 'w') as f:
            f.write(f"# Team Analytics ({start_date} to {end_date})\n\n")
