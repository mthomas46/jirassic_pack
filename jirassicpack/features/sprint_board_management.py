...
def sprint_board_management(jira: Any, params: Dict[str, Any], user_email=None, batch_index=None, unique_suffix=None) -> None:
    ...
    try:
        logger.info(f"[sprint_board_management] Feature entry | User: {user_email} | Params: {redact_sensitive(params)} | Suffix: {unique_suffix}")
        ...
        if not board_name:
            error("board_name is required.", extra={"feature": "sprint_board_management", "user": user_email, "batch": batch_index, "suffix": unique_suffix})
            return
        sprint_name = params.get('sprint_name')
        if not sprint_name:
            error("sprint_name is required.", extra={"feature": "sprint_board_management", "user": user_email, "batch": batch_index, "suffix": unique_suffix})
            return
        ...
        try:
            ...
            error(f"Failed to fetch board or sprint data: {e}. Please check your Jira connection, credentials, and network.", extra={"feature": "sprint_board_management", "user": user_email, "batch": batch_index, "suffix": unique_suffix})
            return
        if not result:
            info("🦖 See, Nobody Cares. No board or sprint data found.", extra={"feature": "sprint_board_management", "user": user_email, "batch": batch_index, "suffix": unique_suffix})
            logger.info("🦖 See, Nobody Cares. No board or sprint data found.")
            return
        ...
        info(f"🌋 Sprint/Board summary written to {filename}", extra={"feature": "sprint_board_management", "user": user_email, "batch": batch_index, "suffix": unique_suffix})
        logger.info(f"[sprint_board_management] Feature complete | Suffix: {unique_suffix}")
    except KeyboardInterrupt:
        logger.warning("[sprint_board_management] Graceful exit via KeyboardInterrupt.")
        info("Graceful exit from Sprint Board Management feature.", extra=context)
    except Exception as e:
        logger.exception(f"[sprint_board_management] Exception: {e}")
        error(f"[sprint_board_management] Exception: {e}", extra={"feature": "sprint_board_management", "user": user_email, "batch": batch_index, "suffix": unique_suffix})
        raise 